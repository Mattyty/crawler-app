...
class CrawlerSupabaseUser extends BaseAuthUser {
  CrawlerSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> crawlerSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CrawlerSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
